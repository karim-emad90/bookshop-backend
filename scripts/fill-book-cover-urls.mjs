...
const STRAPI_URL =
  process.env.STRAPI_URL || 'https://bookshop-backend-xnfc.onrender.com';
...
if (!PEXELS_API_KEY) {
  console.error('❌ Missing PEXELS_API_KEY in back/.env');
  process.exit(1);
}
...
async function run() {
  ...
  const uniqueCategories = [
    ...new Set(books.map((book) => categoryFromSlug(book.slug || ''))),
  ];
  ...
  for (const category of uniqueCategories) {
    ...
    for (let i = 1; i <= 3; i++) {
      try {
        ...
        const imageUrl = await fetchPexelsImageUrl(`${category} book cover`);
        if (!imageUrl) {
          console.log(`  ⚠️ No image found for ${category} (${i}/3)`);
          continue;
        }

        const localFile = await downloadImage(
          imageUrl,
          `${category}-${i}.jpg`
        );
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    try {
      ...
      if (!localFiles?.length) {
        console.log(
          `⚠️ [${i + 1}/${books.length}] No local images for category: ${category}`
        );
        failed++;
        continue;
      }
      ...
      console.log(
        `✅ [${i + 1}/${books.length}] Linked cover: ${slug} | ${title}`
      );
      updated++;
